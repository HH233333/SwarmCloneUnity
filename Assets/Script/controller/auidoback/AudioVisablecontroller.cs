...
public class AudioPeer : MonoBehaviour
{
    ...
    private void CalculateAudioBands()
    {
        for (int freqIndex = 0; freqIndex < freqBands.Length; freqIndex++)
        {
            ...
            audioBandBuffers[freqIndex] = (freqBandBuffers[freqIndex] > 0 ? freqBandBuffers[freqIndex] : 0) / freqBandsHighest[freqIndex];
        }
    }
    ...
    {
        return useBuffer ? (freqBandBuffers[freqIndex] > 0 ? freqBandBuffers[freqIndex] : 0) : freqBands[freqIndex];
    }

    // Normalized frequency band between 0 and 1
    public float GetAudioBand(int freqIndex)
    {
        return useBuffer ? audioBandBuffers[freqIndex] : audioBands[freqIndex];
    }

    public float[] GetSamples(int sampleIndex)
    {
        var samples = samplesList[sampleIndex];
        var max = samplesList[sampleIndex].Max();
        for (int i = 0; i < samples.Length; i++)
            samples[i] = samples[i] / max;
        return samples;
    }

    public float[] GetGetAudioBands()
    {
